...
class GaussianModel:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, iter, N=2, check=False):  # og N=2
        ...
        #print("Split:", selected_pts_mask.sum().item(), "out of", selected_pts_mask.shape[0])
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_split_new(self, grads, grad_threshold, scene_extent, iter, N=2, check=False):
        grad_thresholds = [0.0002, 0.0003, 0.0004, 0.0005]
        Ns = [2, 3, 4, 5]
        ...
        Ns_per_point = torch.zeros_like(padded_grad, dtype=torch.int)
        for i in range(len(grad_thresholds) - 1):
            lower = grad_thresholds[i]
            upper = grad_thresholds[i + 1]
            mask = (padded_grad >= lower) & (padded_grad < upper)
            Ns_per_point[mask] = Ns[i]
        mask = padded_grad >= grad_thresholds[-1]
        Ns_per_point[mask] = Ns[-1]

        selected_pts_mask = Ns_per_point > 0
        ...
        Ns_selected = Ns_per_point[selected_indices]
        max_N = Ns_selected.max().item()

        # Expand indices and repeat for max_N
        expanded_indices = selected_indices.unsqueeze(1).repeat(1, max_N).flatten()  # (total_samples,)

        # Create mask for valid samples according to N per point
        repeat_counts = Ns_selected.unsqueeze(1).repeat(1, max_N)
        sample_mask = torch.arange(max_N, device=repeat_counts.device).unsqueeze(0) < repeat_counts  # (num_pts, max_N)
        sample_mask = sample_mask.flatten()

        # Filter expanded indices with mask
        expanded_indices = expanded_indices[sample_mask]

        # Gather data for expanded indices
        base_xyz = self.get_xyz[expanded_indices]
        base_scaling = self.get_scaling[expanded_indices]
        base_rotation = self._rotation[expanded_indices]
        base_features_dc = self._features_dc[expanded_indices]
        base_features_rest = self._features_rest[expanded_indices]
        base_opacity = self._opacity[expanded_indices]

        # Sample points
        means = torch.zeros_like(base_xyz)
        samples = torch.normal(mean=means, std=base_scaling)

        # Build rotations for all expanded points
        rot = build_rotation(base_rotation)

        # Apply rotation and translate
        new_xyz = torch.bmm(rot, samples.unsqueeze(-1)).squeeze(-1) + base_xyz

        new_scaling = self.scaling_inverse_activation(base_scaling / (0.8 * max_N))
        new_rotation = base_rotation
        new_features_dc = base_features_dc
        new_features_rest = base_features_rest
        new_opacity = base_opacity
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, check=False):
        # Extract points that satisfy the gradient condition
        ...
        #print("Clonned:", selected_pts_mask.sum().item(), "out of", selected_pts_mask.shape[0])
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                                   new_rotation)

    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iter, check=False):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        if iter < 150000:
            self.densify_and_clone(grads, max_grad, extent, check=check)
            self.densify_and_split(grads, max_grad, extent, iter, check=check)
        else:
            max_grad = 0.0005
            self.densify_and_clone(grads, max_grad, extent, check=check)
            self.densify_and_split_new(grads, max_grad, extent, iter, check=check)
        ...
        #print("Final Prunned:", prune_mask.sum().item(), "out of", prune_mask.shape[0])
    ...
    def compute_prune_mask(self, max_grad, min_opacity, extent, max_screen_size, iter, low_density=False, prox=False,
                           ld_iter=2000):
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if max_screen_size:
            big_points_vs = self.max_radii2D > max_screen_size
