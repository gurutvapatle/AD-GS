...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Loss
        check = (iteration > 1500 and stage2)  # 1500
        weight = 0.01
        for i in range(args.gaussiansN):
            depthx = RenderDict[f"depth_gs{i}"]
            imagex = viewpoint_cam.original_image # RenderDict[f"image_gs{i}"]
            depth_loss = edge_aware_smoothness(depthx, imagex)
            range_loss =  - (depthx.max() - depthx.min())
            if not bg_mask is None:
                photo_loss = loss_photometric(RenderDict[f"image_gs{i}"], gt_image, opt=opt, valid=(~bg_mask).float())
                photo_loss += (RenderDict[f"alpha_gs{i}"][bg_mask] ** 2).mean()
                if check:
                    LossDict[f"loss_gs{i}"] = photo_loss + weight * (depth_loss + 0.001 * range_loss)
                else:
                    LossDict[f"loss_gs{i}"] = photo_loss
            else:
                photo_loss = loss_photometric(RenderDict[f"image_gs{i}"], gt_image, opt=opt)
                if check:
                    LossDict[f"loss_gs{i}"] = photo_loss + weight * (depth_loss + 0.001 * range_loss)
                else:
                    LossDict[f"loss_gs{i}"] = photo_loss
        ...
        if not args.onlyrgb:
            if iteration % args.sample_pseudo_interval == 0 and iteration <= args.end_sample_pseudo:
                ...
                #
                #     if iteration >= args.start_sample_pseudo and check:
                #         weight1 = 0.05
                #         depthx1 = RenderDict[f"image_pseudo_co_gs{i}"]
                #         imagex1 = RenderDict[f"depth_pseudo_co_gs{i}"]
                #         depth_loss1 = edge_aware_smoothness(depthx1, imagex1)
                #         range_loss1 = - (depthx1.max() - depthx1.min())
                #         overall_gauss_depth_loss1 = weight1 * (depth_loss1 + 0.001 * range_loss1)
                #         LossDict[f"loss_gs{i}"] += overall_gauss_depth_loss1
                    ...
                    if args.coreg:
                        ...
                        for i in range(args.gaussiansN):
                            for j in range(args.gaussiansN):
                                if i != j:
                                    LossDict[f"loss_gs{i}"] += loss_photometric(RenderDict[f"image_pseudo_co_gs{i}"],
                                                                                RenderDict[
                                                                                    f"image_pseudo_co_gs{j}"].clone().detach(),
                                                                                opt=opt) / (args.gaussiansN - 1)
        ...
        with torch.no_grad():
            ...
            # print("training like a woow == ",progress_bar.format_dict['elapsed'])
            training_time = time.strftime("%H:%M:%S", time.gmtime(progress_bar.format_dict['elapsed']))
            ...
            if dataset.model_path in existing_data:
                existing_data[dataset.model_path]["training_time"] = training_time
            else:
                existing_data[dataset.model_path] = {"training_time": training_time}
            ...
            #saving_iterations = list(range(99,10000,100))
                    ...
                    if iteration<=1500:
                        ...
                        if check:
                            ...
                            for i in range(args.gaussiansN):
                                ...
                                GsDict[f"gs{i}"].densify_and_prune(opt.densify_grad_threshold, opt.prune_threshold,
                                                                   scene.cameras_extent, size_threshold, iteration,check=check)
                        else:
                            #print("High denf")
                            for i in range(args.gaussiansN):
            ...
            if args.coprune and iteration > opt.densify_from_iter and iteration % 500 == 0 and check:
                for i in range(args.gaussiansN):
                    for j in range(args.gaussiansN):
                        if i != j:
                            ...
                            evaluation = o3d.pipelines.registration.evaluate_registration(source_cloud, target_cloud,
                                                                                          threshold, trans_matrix)
...
def training_report(args, tb_writer, iteration, loss, l1_loss, testing_iterations, scene: Scene, renderFunc, renderArgs,
                    GsDict=None):
    if tb_writer:
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if tb_writer and (idx < 8):
                        ...
                        if args.gaussiansN > 1:
                            row2 = torch.cat([render_image_gs1, render_depth_image_gs1, render_opacity_image_gs1],
                                             dim=2)
                        else:
                            row2 = torch.cat([black, black, black], dim=2)
                        ...
                        tb_writer.add_images(config['name'] + "_view_{}/render_image".format(viewpoint.image_name),
                                             render_image[None], global_step=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int,
                        default=[500, 2000, 3000, 5000, 7000, 10000, 15000, 30000])
